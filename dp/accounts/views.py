...
def account_view(request):
    if request.user.is_authenticated:
        ...
        orders = Orders.objects.filter(user_bound=user.id)
        ...
        if request.method == "POST":
            user_form = UserAccountForm(
                request.POST or None, instance=request.user, initial=initial_data
            )
            profile_form = ProfileForm(
                request.POST or None,
                instance=request.user.profile,
                initial=initial_data_profile,
            )
...
def register_view(request):
    ...
    if form.is_valid():
        print("Form is valid")
        if form.cleaned_data.get("captcha") == "7":
            print(form.cleaned_data.get("captcha"))
